...
function checkForGameEnd() {
  ...
  for (let i = 0; i < 3; i++) {
    if (
      board.value.state[i][0] == board.value.state[i][1] &&
      board.value.state[i][1] == board.value.state[i][2] &&
      board.value.state[i][0] != 0
    ) {
    ...
    if (
      board.value.state[0][i] == board.value.state[1][i] &&
      board.value.state[1][i] == board.value.state[2][i] &&
      board.value.state[0][i] != 0
    ) {
  ...
  if (
    board.value.state[0][0] == board.value.state[1][1] &&
    board.value.state[1][1] == board.value.state[2][2] &&
    board.value.state[0][0] != 0
  ) {
  ...
  if (
    board.value.state[0][2] == board.value.state[1][1] &&
    board.value.state[1][1] == board.value.state[2][0] &&
    board.value.state[0][2] != 0
  ) {
...
async function playAgain() {
  ...
  const shuffle =
    'fU1rR3QCDteqJbZ7iucTzGFaApgsO9YKHEwoyMk5lWIv2nS8P6LNX4BVxmd0jh'
  let newId = ''
...
<template>
  <div class="bg-gray-300 h-full">
    ...
    <div
      v-if="isRunning"
      class="flex w-full justify-center items-center flex-col mb-2 mt-2 space-y-1 lg:space-y-3"
    >
      <div
        v-for="(row, row_index) in board.state"
        class="w-full flex justify-center space-x-1 lg:space-x-3"
      >
        <Field
